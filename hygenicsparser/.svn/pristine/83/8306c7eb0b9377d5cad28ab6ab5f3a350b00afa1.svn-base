	...
	private void iter() {
		...
		for (String table : schema_columns.keySet()) {
			...
			try {
				...
				try {
					throw new SQLMalformedException("FATAL ERROR: Table name "
							+ table + " malformed");
			...
			if (template.checkTable(table, schema)) {
				...
				for (int i = 0; i < columns.size(); i++) {
					...
					if (tablecolumns.contains(columns.get(i).trim()) == false
							|| (insensitive == true && tablecolumns
									.contains(columns.get(i).toLowerCase()
											.trim()) == false)) {

						if (correct) {
							int dist = Levenshtein.getLevenshteinDistance(tablecolumns.get(i), columns.get(i));
							add = false;
							log.info("Distance was " + dist + " Cutoff is "
									+ levcutoff);
