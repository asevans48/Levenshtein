 ...
 *
 */
public class JDumpWithReference {
	...
	// set these for the base paths in table and file
	...
	public String getDelimreplace() {
		return delimreplace;
	}

	public void setDelimreplace(String delimreplace) {
		this.delimreplace = delimreplace;
	}
	...
	private void toFile() {
		...
		for (String table : fpaths.keySet()) {
			int offset = 0;
			if (template.checkTable(this.baseschema + "." + table,
					this.baseschema)) {
				if (template.getCount(this.baseschema + "." + table) > 0) {
					log.info("Dumping for " + table);
					// get header
					String select = "SELECT * FROM " + this.baseschema + "."
							+ table;
					String fpath = null;
					...
					if (headers != null && fpath != null) {
						...
						if (!new File(fpath).exists()) {
							...
							try {
								new File(this.baseFilePath + fpath)
										.createNewFile();
							} catch (IOException e) {
								e.printStackTrace();
							}
